...
def getCryptoModel(ticker, numDays, initial_capital):
        ...
        stock_df = get_crypto_bars(symbol=symbol, timeframe=timeframe, start=start, end=end, limit=limit)
        ...
        # %%
        combined_accuracy = pd.concat([svm_training_report, gbc_training_report, sgdc_training_report, lta_training_report, nvc_training_report, lr_training_report, knc_training_report, rfc_training_report, dtc_training_report], axis=1)
        ...
        # %%
        combined_preds.columns = ['svm_preds', 'gbc_preds', 'lr_preds','sgdc_preds', 'dtc_preds', 'lta_preds', 'rfc_preds', 'knc_preds', 'nvc_preds']

        # %%
        # Calculating the mean prediction values across the models
        combined_preds['mean'] = (combined_preds['svm_preds'] + combined_preds['gbc_preds'] + combined_preds['lr_preds'] + combined_preds['sgdc_preds']  + combined_preds['dtc_preds']  + combined_preds['lta_preds'] + combined_preds['rfc_preds'] + combined_preds['knc_preds'] + combined_preds['nvc_preds']) / 9
        ...
        share_size = (initial_capital / (combined_preds['close'] * 1.1)).round(2)

        combined_preds['position'] = (share_size * (combined_preds['signals'])).round(2)
    ...
    # %%
        ...
        print(f'Portfolio standard deviation is str(combined_preds["Portfolio Daily Returns"].std())')
